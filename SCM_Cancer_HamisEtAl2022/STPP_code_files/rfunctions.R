...
print(xlim)
 ...
 if (dim == 1) {
  ...
  #print(tabulate(floor(x[,1]/grid.resolution)*gn + floor(x[,2]/grid.resolution) + 1, nbins=gn*gn))
  ...
  #print("gcount:")
  #print(gcount)
  #print("floors x/bin_size(i.e grid resolution):")
  #print( floor(x[,1]/grid.resolution) )
  #print("ans * no_bins_1direction (i.e. gn)  :")
  #print( floor(x[,1]/grid.resolution)*gn )
  #print("ans +  floor(x[,2]/grid.resolution) + 1 :")
  #print( floor(x[,1]/grid.resolution)*gn + floor(x[,2]/grid.resolution) )  #so (x,y) becomes n=x*no_bins + y +1
  print( tabulate( floor(x[,1]/grid.resolution)*gn + floor(x[,2]/grid.resolution) ))  #...
       #1D array for n that is 1 for every coord where there is a cell

  print("gcount:")
  print(gcount)  #2d array for the same
  ...
  print("gcount length")
  print(length(gcount)) 
  print(" Re(fft(abs(fft(gcount))^2, inverse=T))/length(gcount)")
...
plot.spm2 = function(coord,species,U,grid.resolution=1,selected.species=NULL,pch=NULL,col=NULL,cex=NULL,xlim=NULL,ylim=NULL,xlab="Distance",ylab=NULL, cumulant=F, ...) {
 ...
 if (is.null(selected.species)) {
  ...
  #sprintf("cex vector length (%d) does not match with selected.species (%d).\n",length(cex),
 }
 ...
 #print(xlim) #half of U
 #print(U)
 
 #print(dim(coord)) #dimension, how many coordinates
 #print(coord)
 ...
 #print(dimension) #dimension is the number of cols (2)
 
 minval = 0
 maxval = 0
 for (si in 1:length(selected.species)) {  #first for 1, then for 2
  i=which(species == selected.species[si]) #takes out the row numver
#  print("i=")
 # print(i)
  if (dimension == 1) {
   ...
   s[[si]]=spm2(coord[i,,drop=F], U=U, grid.resolution=grid.resolution,xlim=xlim, cumulant=cumulant) #computes smp2 contr for everty point
#	print("si= ")
#	print(si)
#	print(i)
 #       print("cordi =")
#	print(coord[i,,drop=F]) #all the coordinates for species i 
#	print(grid.resolution)
#	print(xlim)
#	print(cumulant)
 #	print(s)
  }
 #get min and max to set ylimits in plot
  mm = max(s[[si]]$spm2)
  #print("mm=")
  #print(mm)
  if (mm > maxval) {
 ...
 if (length(selected.species) > 1) {
  for (si in 2:length(selected.species)) {
   points(s[[si]]$dist,s[[si]]$spm2,pch=pch[si],col=col[si],cex=cex[si]) #plot more if needed
  }
 }
}
